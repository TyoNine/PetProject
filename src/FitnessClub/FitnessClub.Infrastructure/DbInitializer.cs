...
namespace FitnessClub.Infrastructure
{
    public class DbInitializer : IDbInitializer
    {
        ...
        public void Initialize(FitnessClubContext context)
        {
            ...
            {
                new Gym { Description="A" },
                new Gym { Description="B" },
                new Gym { Description="C" }
            };

            DateTime date = DateTime.Today.AddDays(4);
            List<Booking> bookings = new List<Booking>
            {
                new Booking { StartDate=date, EndDate=date.AddDays(14), IsActive=true, CustomerId=1, GymId= 1 },
                new Booking { StartDate=date, EndDate=date.AddDays(14), IsActive=true, CustomerId=2, GymId=2 },
                new Booking { StartDate=date, EndDate=date.AddDays(14), IsActive=true, CustomerId=1, GymId=3 }
            };

            context.Customer.AddRange(customers);
            context.Gym.AddRange(rooms);
            context.SaveChanges();
